...
date_sel.datepicker({
    onSelect: function (selected_date, datepicker) {
        //get the date formats consistent across all functions.
        var sel_date = moment(selected_date).format('MM DD YYYY');
        buildCalendar(sel_date);
...
function buildCalendar(t_date) {
    ...
    var mcheck = moment().format('MM DD YYYY');
    ...
    for (i = 0; i < 13; i++) {
        ...
        if (mcheck > selected_date) {
            meeting_cat = 'past';
        }
        else if (mcheck < selected_date) {
            meeting_cat = 'future';
        }
        else {
